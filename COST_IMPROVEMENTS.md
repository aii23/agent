...
| G | **Synthesis decoupled from manager persona.** Single shared `SYNTHESIS_SYSTEM_PROMPT` for all managers (CEO/CPO/CMO/CTO/CFO/CLO). Manager persona prompts are now used only at planning time — they no longer leak planner-side template language ("→ cpo-reviewer only if...", recipe shapes, etc.) into the synthesis output as simulated agent transcripts. Manager-specific synthesis tone/format flows via the planner-authored `synthesisPrompt` for that turn. Schema description on `synthesisPrompt` updated to make the contract explicit | `orchestrator/synthesize.ts`, `orchestrator/plan.ts` | Quality fix; eliminates a class of synthesis hallucinations (fake step headers, simulated reviewer dialogs, "Now sending to..." narration) |
...
| Phase 2: shared `SYNTHESIS_SYSTEM_PROMPT`, not per-manager synthesis prompts | Two reasons: (1) most "How to synthesize" guidance across managers is the same advice ("lead with the answer, show trade-offs"); the manager-specific bits should ride on the planner-authored `synthesisPrompt`, which is already turn-tailored. (2) Maintaining 6 separate synthesis prompts would drift; one prompt is the source of truth for output discipline. Trade-off: lose direct manager-voice control at synthesis time, but the planner already encodes voice in the per-turn synthesisPrompt (e.g., "professional, not hype-y", "ADR shape", "show the working"). |
| Phase 2: kept "How to synthesize" sections in manager persona prompts | They become planner-side guidance now (the planner reads them when writing each turn's synthesisPrompt), which is the right place for manager-specific voice/format hints. The synthesizer never reads them, so the leak vector is closed regardless. |
