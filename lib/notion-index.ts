...
function richTextToPlain(items: RichTextItemResponse[] | string): string {
  if (!Array.isArray(items) && typeof items !== "string") {
    console.warn(
      "[notion-index] richTextToPlain: expected array, got",
      typeof items,
...
function extractBlockText(block: BlockObjectResponse): string {
  ...
  if (!data) {
    console.warn(
      "[notion-index] extractBlockText: no data for block type",
      type,
      "id",
      block.id,
    );
 ...
 */
export async function syncNotionIndexIncremental(): Promise<{
  synced: number;
}> {
  ...
  console.log(
    `[notion-index] Incremental sync — checking pages edited after ${since.toISOString()}`,
  );
  ...
  console.log(
    `[notion-index] Incremental sync complete — ${synced} pages updated`,
  );
  return { synced };
}
...
export function formatNotionIndex(pages: NotionPageIndex[]): string {
  ...
  return pages.map((p) => `[${p.id}] ${p.path}\n  → ${p.summary}`).join("\n");
}
